...
// Remove unused hasImageConnection variable
  ...
  return (
    <div className={`w-72 bg-black/60 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.8)] p-0 font-sans group hover:border-white/20 transition-all duration-300 ${data.isGenerating ? "border-purple-500/50 shadow-[0_0_30px_rgba(168,85,247,0.4)] animate-pulse" : ""}`}>
      ...
      <div className="p-3 flex flex-col gap-3 relative">
        <div className="relative flex items-center h-4">
          <Handle type="target" id="image_url" position={Position.Left} style={{ left: "-8px" }} style={{ left: "-8px" }} className="w-3! h-3! border-0! bg-white! shadow-[0_0_12px_rgba(255,255,255,0.9)]! ring-2! ring-black/50!" />
          <span className="text-[10px] text-krea-text-muted uppercase ml-1">Image URL *</span>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div className="relative">
            <Handle type="target" id="x_percent" position={Position.Left} style={{ left: "-8px" }} style={{ left: "-8px" }} className="w-3! h-3! border-0! bg-white! shadow-[0_0_12px_rgba(255,255,255,0.9)]! ring-2! ring-black/50!" />
          ...
          <div className="relative">
            <Handle type="target" id="y_percent" position={Position.Left} style={{ left: "-8px" }} style={{ left: "-8px" }} className="w-3! h-3! border-0! bg-white! shadow-[0_0_12px_rgba(255,255,255,0.9)]! ring-2! ring-black/50!" />
            <label className="flex flex-col text-[10px] text-krea-text-muted gap-1 ml-1">
              Y (%)
              <input 
                type="number" 
          ...
          <div className="relative">
            <Handle type="target" id="width_percent" position={Position.Left} style={{ left: "-8px" }} style={{ left: "-8px" }} className="w-3! h-3! border-0! bg-white! shadow-[0_0_12px_rgba(255,255,255,0.9)]! ring-2! ring-black/50!" />
            <label className="flex flex-col text-[10px] text-krea-text-muted gap-1 ml-1">
              Width (%)
              <input 
                type="number" 
          ...
          <div className="relative">
            <Handle type="target" id="height_percent" position={Position.Left} style={{ left: "-8px" }} style={{ left: "-8px" }} className="w-3! h-3! border-0! bg-white! shadow-[0_0_12px_rgba(255,255,255,0.9)]! ring-2! ring-black/50!" />
            <label className="flex flex-col text-[10px] text-krea-text-muted gap-1 ml-1">
              Height (%)
              <input 
                type="number" 
        ...
        {data.output && (
          <div className="mt-2 text-[10px] text-krea-accent truncate" title={data.output}>
            Output: {data.output}
          </div>
        )}
      </div>

      <Handle type="source" id="output" position={Position.Right} style={{ right: "-8px" }} style={{ right: "-8px" }} className="w-3! h-3! border-0! bg-white! shadow-[0_0_12px_rgba(255,255,255,0.8)]! ring-2! ring-black/50!" style={{ top: '50%' }} />
    </div>
  );
}
