...
import { VideoIcon, X , Play, RefreshCw } from 'lucide-react';
...
function VideoNode({ id, data }: { id: string; data: any }) {
    ...
    <div className="w-80 bg-black/60 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.8)] p-0 font-sans group hover:border-white/20 transition-all duration-300">
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-gradient-to-b from-white/[0.04] to-transparent">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-[11px] tracking-wide text-white/80 uppercase">Video</span>
        </div>
      ...
      <div className="p-5 flex flex-col gap-4 relative">
        ...
        >
          {data.url ? (
            <>
              <video src={data.url} controls className="absolute inset-0 w-full h-full object-cover" />
              <button
                onClick={(e) => { 
                  e.preventDefault(); 
                  e.stopPropagation(); 
            ...
            <div 
              className="w-full h-full flex flex-col items-center justify-center p-4 hover:bg-neutral-800 transition-colors cursor-pointer"
              onClick={(e) => {
                e.preventDefault();
                uppy.cancelAll();
                setIsOpen(true);
