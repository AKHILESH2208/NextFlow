...
// Remove unused hasVideoConnection variable
  ...
  return (
    <div className={`w-56 bg-[#0f0f0f] text-white border border-white/10 rounded-xl shadow-[0_0_30px_rgba(0,0,0,1)] p-0 font-sans transition-all duration-300 ${data.isGenerating ? 'ring-2 ring-krea-accent shadow-[0_0_20px_var(--color-krea-accent)] animate-pulse' : ''}`}>
      ...
      <div className="p-3 flex flex-col gap-3 relative">
        <div className="relative flex items-center h-4">
          <Handle type="target" id="video_url" position={Position.Left} style={{ left: "-8px" }} style={{ left: "-8px" }} className="w-3! h-3! border-0! bg-white! shadow-[0_0_12px_rgba(255,255,255,0.9)]! ring-2! ring-black/50!" />
          <span className="text-[10px] text-krea-text-muted uppercase ml-1">Video URL *</span>
        </div>

        <div className="relative">
          <Handle type="target" id="timestamp" position={Position.Left} style={{ left: "-8px" }} style={{ left: "-8px" }} className="w-3! h-3! border-0! bg-white! shadow-[0_0_12px_rgba(255,255,255,0.9)]! ring-2! ring-black/50!" />
